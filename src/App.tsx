...
export default function App() {
  ...
  const handleFileChange = useCallback(async (type: MainFileType, file: File) => {
    ...
    try {
      ...
      // Для CSV пробуем форсировать разделитель, если определим его по расширению
      ...
      // Проверка на неправильный разделитель в CSV (если все в одной колонке)
      ...
      // Ищем строку заголовка (ищем строку, где больше всего совпадений с терминами)
      ...
      const jsonData = rawData.slice(headerRowIndex + 1).filter(row => {
        ...
        // Принудительно приводим к простому виду: только 3 столбца
        return {
          '__col_0': cleanGeneral(row[0]), // Регистрационный номер
          '__col_1': cleanGeneral(row[1]), // Порядковый номер (Ключ)
          '__col_2': cleanGeneral(row[2])  // Наименование
        };
      });
  ...
  const handleAccumulatorReportFile = useCallback(async (file: File) => {
  ...
  const handleWinterDecoderFile = useCallback(async (file: File) => {
  ...
  const handleSummerDecoderFile = useCallback(async (file: File) => {
